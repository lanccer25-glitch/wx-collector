...
class Db:
    ...
    def __init__(self,tag:str="默认",User_In_Thread=True):
        ...
        self.init(self._resolve_db_url())
    def _resolve_db_url(self) -> str:
        """Resolve the database URL.
        If DATABASE_URL points to PostgreSQL, always use it (both dev and prod).
        This ensures development and production share the same database.
        Falls back to the configured db URL (SQLite default) only when DATABASE_URL
        is not set or not a PostgreSQL connection.
        """
        import os
        db_url = cfg.get("db") or "sqlite:///data/db.db"
    ...
    def add_article(self, article_data: dict,check_exist=False) -> bool:
        try:
            ...
            if (
                "UNIQUE" in e_str
                or "Duplicate entry" in e_str
                or "duplicate key" in e_str.lower()
                or "UniqueViolation" in e_str
                or "unique constraint" in e_str.lower()
            ):
                return False
            else:
