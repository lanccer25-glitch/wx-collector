...
async def get_current_user(token: str = Depends(oauth2_scheme)):
    ...
    if os.getenv("REQUIRE_LOGIN", "False").strip().lower() not in ("true", "1", "yes"):
        user = get_user("admin")
        if user:
            return {"username": user.username, "role": user.role or "admin",
                    "permissions": user.permissions, "original_user": user}
        return {"username": "admin", "role": "admin", "permissions": None, "original_user": None}
...
async def get_current_user_or_ak(request: Request, token: str = Depends(oauth2_scheme)):
    ...
    if os.getenv("REQUIRE_LOGIN", "False").strip().lower() not in ("true", "1", "yes"):
        ...
        if user:
            return {"username": user.username, "role": user.role or "admin",
                    "permissions": user.permissions, "original_user": user, "auth_type": "bypass"}
        return {"username": "admin", "role": "admin", "permissions": None, "original_user": None, "auth_type": "bypass"}
