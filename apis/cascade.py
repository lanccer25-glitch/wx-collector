...
async def heartbeat(
    ...
    try:
        ...
        if auth_header.startswith("AK-SK "):
            ...
            node_id_from_header = credentials.split(':')[1] if ':' in credentials else None

            # 查找对应节点并更新状态，不存在则自动注册
            session = DB.get_session()
            try:
                ...
                if node:
                    ...
                    new_node_id = node_id_from_header or str(_uuid.uuid4())
                    node_name = (body or {}).get("node_name", f"auto-{api_key[:8]}")
                    ...
                    session.commit()
                    print_info(f"[Heartbeat] 自动注册子节点: {node_name} (id: {new_node_id}, key: {api_key[:8]}...)")
            except Exception as inner_e:
                try:
                    session.rollback()
                except Exception:
                    pass
                print_error(f"心跳更新节点状态失败: {str(inner_e)}")
            finally:
                try:
                    session.close()
                except Exception:
...
async def upload_articles(
    ...
    try:
        ...
        from tools.fix import fix_html as _fix_html
        ...
        for article_data in req.articles:
            ...
            if not existing:
                ...
                article = Article(
                    ...
                    content_html=_fix_html(raw_content) if raw_content else None,
