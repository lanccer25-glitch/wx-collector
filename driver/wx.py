...
class Wx:
    ...
    def QRcode(self):
        return {
            "code":f"/api/v1/wx/auth/qr/png?t={(time.time())}",
    ...
    def QrStatus(self):
        ...
        if not login_status:
            try:
                ...
                pass
        return {"login_status": login_status, "qr_code": self.GetHasCode()}
    ...
    def wxLogin(self, CallBack=None, NeedExit=True):
        ...
        try:
            ...
            try:
                ...
                from core.models.config_management import ConfigManagement as _CM
                with open(self.wx_login_url, 'rb') as _f:
                    _b64_str = _b64.b64encode(_f.read()).decode('utf-8')
                _db_s = _DB.get_session()
                _ex = _db_s.query(_CM).filter(_CM.config_key == 'wx_qrcode_b64').first()
                if _ex:
                    _ex.config_value = _b64_str
                else:
                    _db_s.add(_CM(config_key='wx_qrcode_b64', config_value=_b64_str))
                _db_s.commit()
                print("二维码已同步到数据库")
            except Exception as _e:
                print(f"二维码同步数据库失败: {_e}")
