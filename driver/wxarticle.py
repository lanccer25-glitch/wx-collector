...
class WXArticleFetcher:
    ...
    def get_article_content(self, url: str) -> Dict:
        ...
        try:
            ...
            # 拦截微信 getappmsgext 接口，捕获阅读量/点赞数
            def _handle_response(response):
                try:
                    if "getappmsgext" in response.url and response.status == 200:
                        data = response.json()
                        stat = data.get("appmsgstat", {})
                        if stat:
                            stats_data.update(stat)
                            print_info(f"获取文章统计: 阅读={stat.get('read_num',0)} 点赞={stat.get('like_num',0)}")
                except Exception:
                    pass
            ...
            info["read_num"]=stats_data.get("read_num", 0)
            info["like_num"]=stats_data.get("like_num", 0)
            info["old_like_num"]=stats_data.get("old_like_num", 0)
            info["share_num"]=stats_data.get("share_num", 0)
        ...
        try:
            ...
            self.Close()
        return info
    def fetch_stats_only(self, url: str) -> dict:
        """仅获取文章统计数据（阅读/点赞/在看/转发），不抓取正文内容。
        
        注意：微信 getappmsgext API 需要真实微信用户凭据（uin+key）才返回 appmsgstat。
        使用移动端模式触发该 API 调用，并用 expect_response 同步等待响应，避免竞争条件。
        """
        empty = {"read_num": 0, "like_num": 0, "old_like_num": 0, "share_num": 0}
        try:
            # 使用移动端模式——文章页面在移动端会自动调用 getappmsgext
            self.controller.start_browser(proxy_url=self.browser_proxy_url, mobile_mode=True)
            self.page = self.controller.page
            page = self.page
            ...
            stat = {}
            try:
                # expect_response 在 open_url 之前设置，确保不错过响应
                with page.expect_response(
                    lambda r: "getappmsgext" in r.url,
                    timeout=12000
                ) as resp_info:
                    self.controller.open_url(url)
                ...
                if appmsgstat:
                    stat = appmsgstat
                    print_info(f"[统计刷新] 阅读={stat.get('read_num',0)} 点赞={stat.get('like_num',0)}")
                else:
                    print_warning(f"[统计刷新] getappmsgext 无 appmsgstat (ret={data.get('base_resp',{}).get('ret')}): {url[:50]}")
            except PWTimeout:
                print_warning(f"[统计刷新] 等待 getappmsgext 超时，文章可能不支持统计: {url[:50]}")

            return {
                "read_num": stat.get("read_num", 0),
                "like_num": stat.get("like_num", 0),
                "old_like_num": stat.get("old_like_num", 0),
                "share_num": stat.get("share_num", 0),
            }
        except Exception as e:
            print_error(f"[统计刷新] 失败: {e}")
            return empty
        finally:
            self.Close()
