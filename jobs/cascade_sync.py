...
class CascadeSyncService:
    ...
    async def claim_and_execute_task(self):
        ...
        try:
            ...
            # 2. 执行任务（返回 (results, articles_with_content)）
            results, articles = await self._execute_task(task_package)
            ...
            if articles:
                try:
                    print_info(f"上行 {len(articles)} 篇带正文文章到父节点...")
                    await self.client.upload_articles(
                        allocation_id=allocation_id,
                        articles=articles,
                    )
                    print_success(f"文章上行成功: {len(articles)} 篇")
                except Exception as upload_err:
                    print_error(f"文章上行失败（不影响完成上报）: {upload_err}")
    ...
    async def _execute_task(self, task_package: dict):
        """
        执行认领到的任务
        ...
        返回: (results: list, articles_with_content: list)
            results              — 每个公众号的执行状态摘要
            articles_with_content — 本次新增文章（含正文），用于上行父节点
        """
        from jobs.mps import do_job
        from core.models.feed import Feed
        from core.models.message_task import MessageTask
        from core.models.article import Article as ArticleModel
        from core.article_content import fetch_article_content, build_article_url
        ...
        for feed_data in feeds_data:
            try:
                ...
                # ── 记录 do_job 启动时间，用于查询新增文章 ──
                import threading
                from datetime import datetime as _dt
                job_start = _dt.utcnow()
                ...
                try:
                    art_session = DB.get_session()
                    try:
                        new_arts = art_session.query(ArticleModel).filter(
                            ArticleModel.mp_id == feed_id,
                            ArticleModel.created_at >= job_start,
                        ).all()

                        new_articles_count = len(new_arts)
                        if new_arts:
                            print_info(f"[{feed_name}] 本次新增文章 {new_articles_count} 篇，开始抓取正文...")

                        for art in new_arts:
                            art_url = build_article_url(art)
                            if art_url and not (art.content or "").strip():
                                try:
                                    content, mode, extra = fetch_article_content(
                                        art_url, preferred_mode="web"
                                    )
                                    if content and content != "DELETED":
                                        art.content = content
                                        art.content_html = fix_html(content)
                                        art_session.commit()
                                        art_session.refresh(art)
                                        print_info(
                                            f"正文抓取成功 [{(art.title or art.id)[:25]}] "
                                            f"{len(content)} 字 (模式: {mode})"
                                        )
                                    else:
                                        print_warning(
                                            f"正文为空 [{(art.title or art.id)[:25]}] "
                                            f"mode={mode}"
                                        )
                                except Exception as content_err:
                                    print_error(
                                        f"正文抓取失败 [{(art.title or art.id)[:25]}]: {content_err}"
                                    )
                        ...
                        result_container["article_count"] = new_articles_count
                        result_container["new_article_count"] = new_articles_count
                    finally:
                        art_session.close()
                except Exception as query_err:
                    print_error(f"查询新增文章失败 [{feed_name}]: {query_err}")
                ...
                if result_container["success"]:
                    print_success(f"公众号 {feed_name} 更新完成，新增 {new_articles_count} 篇")
                else:
        ...
        print_success(f"任务执行完成，处理 {len(results)} 个公众号，共收集 {len(all_articles)} 篇文章")
