...
class CascadeTaskDispatcher:
    ...
    def _create_pending_allocation_by_ids(
        self,
        task_id: str,
        task_name: str,
        cron_exp: str,
        ...
        """
        创建待认领的任务分配记录（接受 feed ID 字符串列表，避免 session 冲突）
        """
        session = DB.get_session()
        try:
            allocation_id = str(uuid.uuid4())
            allocation = CascadeTaskAllocation(
                ...
                cron_exp=cron_exp,
                node_id=None,
                feed_ids=json.dumps(feed_id_list),
                status='pending',
                schedule_run_id=schedule_run_id,
                dispatched_at=datetime.utcnow()
            )
            session.add(allocation)
            session.commit()
            print_success(f"创建待认领任务: {len(feed_id_list)}个公众号 (allocation_id: {allocation_id})")
            return allocation
        except Exception as e:
            session.rollback()
            print_error(f"创建待认领任务失败: {str(e)}")
    ...
    def dispatch_task_to_children(
        ...
        # Step 1: 获取所有 feed ID（在独立 session 中，避免与内层 session 冲突）
        task_id = task.id
        task_name = task.name
        cron_exp = task.cron_exp
        ...
        try:
            ...
            if not feeds:
                ...
                if feeds:
                    print_info(f"任务 {task_name} 未指定公众号，自动使用全部 {len(feeds)} 个公众号")
                else:
                    print_warning(f"任务 {task_name} 没有关联公众号")
                    return False

            print_info(f"任务 {task_name} 包含 {len(feeds)} 个公众号")
            # 关键：提前提取 ID 为纯字符串列表，避免 ORM 对象随 session 失效
            all_feed_ids = [f.id for f in feeds]
            total_feeds = len(all_feed_ids)
        except Exception as e:
            print_error(f"查询公众号失败: {str(e)}")
            return False
        finally:
            session.close()  # 关键：先关闭外层 session，再开始创建分配

        # Step 2: 循环创建 allocation，每次使用独立 session（不会冲突）
        allocations = []
        created_count = 0

        for i in range(0, total_feeds, feeds_per_allocation):
            batch_ids = all_feed_ids[i:i + feeds_per_allocation]

            allocation = self._create_pending_allocation_by_ids(
                task_id, task_name, cron_exp, batch_ids, schedule_run_id
            )

            if allocation:
                allocations.append(allocation)
                created_count += len(batch_ids)
                if (i // feeds_per_allocation + 1) % 10 == 0 or i == 0:
                    print_info(f"创建任务分配进度: {i // feeds_per_allocation + 1}/{(total_feeds + feeds_per_allocation - 1) // feeds_per_allocation}")
        ...
        if allocations:
            ...
            print_warning(f"任务 {task_name} 记录创建失败")
            return False
