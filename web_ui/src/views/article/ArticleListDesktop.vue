            ...
            <a-list :data="filteredMpList" :loading="mpLoading" bordered>
              <template #item="{ item, index }">
                <a-list-item @click="handleMpClick(item.id)" :class="{ 'active-mp': activeMpId === item.id }"
                  @mouseenter="hoveredMpId = item.id" @mouseleave="hoveredMpId = ''"
                  ...
                  <div style="display: flex; align-items: center;">
                    ...
                    <a-button v-if="hoveredMpId === item.id && canManageMp(item.id)" size="mini" type="text" status="danger"
                      @click="$event.stopPropagation(); deleteMp(item.id)">
                      <template #icon><icon-delete /></template>
                    </a-button>
                    <a-button v-if="hoveredMpId === item.id && canManageMp(item.id)" size="mini" type="text"
      ...
      <a-layout-content :style="{ padding: '20px', width: '100%' }">
        <a-page-header :title="activeFeed ? activeFeed.name : '全部'" :subtitle="'管理您的公众号订阅内容'" :show-back="false">
          <template #extra>
            <a-space>
              ...
              <a-button
                v-if="activeFeed?.id != '' && activeFeed?.id !== FEATURED_MP_ID"
                :loading="notionSyncLoading"
                @click="handleNotionSync"
              >
                同步到Notion
              </a-button>
              <a-dropdown>
        ...
        <a-card style="border:0">
          ...
          <a-table :columns="columns" :data="articles" :loading="loading" :pagination="pagination" :row-selection="{
            ...
            checkStrictly: true,
            onlyCurrent: false
          }" row-key="id" @page-change="handlePageChange" @page-size-change="handlePageSizeChange" @sorter-change="handleSorterChange" v-model:selectedKeys="selectedRowKeys">
          ...
          <a-modal v-model:visible="refreshModalVisible" title="账号刷新" :width="380">
            <a-form :model="refreshForm">
              <a-form-item label="抓取范围">
                <a-radio-group v-model="refreshForm.timeRange" direction="vertical">
                  <a-radio value="7">近一周</a-radio>
                  <a-radio value="30">近一月</a-radio>
                  <a-radio value="180">近半年</a-radio>
                  <a-radio value="365">近一年</a-radio>
                  <a-radio value="0">全部历史文章（耗时较长）</a-radio>
                </a-radio-group>
              </a-form-item>
            </a-form>
            <template #footer>
...
import { getArticles, deleteArticle as deleteArticleApi, ClearArticle, ClearDuplicateArticle, getArticleDetail, getRefreshArticleTaskStatus, refreshArticle as refreshArticleApi, toggleArticleFavoriteStatus, toggleArticleReadStatus, batchRefreshStats, getBatchStatsStatus, notionSyncMp } from '@/api/article'
...
import { refreshAllMps } from '@/api/sysInfo'
...
const handleSorterChange = (dataIndex: string, direction: string) => {
  ...
  sortBy.value = fieldMap[dataIndex] || 'publish_time'
  sortOrder.value = direction === 'ascend' ? 'asc' : 'desc'
  pagination.value.current = 1
  fetchArticles()
}
...
const notionSyncLoading = ref(false)

const handleNotionSync = async () => {
  const mpId = activeMpId.value
  if (!mpId || mpId === FEATURED_MP_ID) return
  notionSyncLoading.value = true
  try {
    const res = await notionSyncMp(mpId)
    const msg = (res as any)?.data?.message || (res as any)?.message || '\u5df2\u5f00\u59cb\u540c\u6b65\u5230Notion'
    Message.success(msg)
  } catch (error: any) {
    Message.error(error?.message || '\u540c\u6b65\u5230Notion\u5931\u8d25')
  } finally {
    notionSyncLoading.value = false
  }
}

const batchStatsLoading = ref(false)
const batchStatsTooltip = ref('批量刷新所有文章的统计数据')
let _statsTimer: ReturnType<typeof setInterval> | null = null

const _pollBatchStats = async () => {
  try {
    const res = await getBatchStatsStatus()
    const d = res?.data?.data
    if (!d) return
    const { status, done, total } = d
    batchStatsTooltip.value = `刷新统计: ${done||0}/${total||0}`
    if (status === 'done' || status === 'failed') {
      batchStatsLoading.value = false
      batchStatsTooltip.value = status === 'done' ? `统计已更新 ${total||0} 篇` : '刷新失败'
...
const handleBatchRefreshStats = async () => {
  if (batchStatsLoading.value) return
  batchStatsLoading.value = true
  batchStatsTooltip.value = '刷新统计中...'
  try {
    await batchRefreshStats()
    _statsTimer = setInterval(_pollBatchStats, 3000)
  } catch {
    batchStatsLoading.value = false
  }
}
...
const handleRefresh = () => {
  ...
  const days = parseInt(refreshForm.value.timeRange)
  const params: { start_page?: number; end_page?: number; since_ts?: number } = {}
  if (days > 0) {
    params.since_ts = Math.floor((Date.now() / 1000) - days * 86400)
    params.end_page = Math.ceil(days / 5) + 2
  } else {
    params.start_page = 0
    params.end_page = 200
  }
  UpdateMps(activeMpId.value, params).then(() => {
    Message.success('已开始抓取，请稍后手动刷新查看结果')
    refreshModalVisible.value = false
  }).catch((err: any) => {
    const msg = err?.message || (typeof err === 'string' ? err : '刷新失败，请稍后重试')
    Message.error(msg)
  }).finally(() => {
...
const handleRefreshAll = async () => {
  ...
  try {
    const res = await refreshAllMps()
    Message.success(res?.message || '已开始后台采集，请稍后刷新查看最新文章')
  } catch (e: any) {
    Message.error(e?.message || '采集启动失败，请稍后再试')
  } finally {
