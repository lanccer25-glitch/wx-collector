...
// 更新订阅公众号文章列表 
export const UpdateMps = (mp_id: string,params: { start_page?: number; end_page?: number; since_ts?: number }) => {
   const apiParams = {
    start_page: (params?.start_page || 0),
    end_page: params?.end_page || 1,
    since_ts: params?.since_ts || 0
  }
  let url = `/wx/mps/update/${mp_id||'all'}?start_page=${apiParams.start_page}&end_page=${apiParams.end_page}`
  if (apiParams.since_ts > 0) url += `&since_ts=${apiParams.since_ts}`
  return http.get<{code: number, message: string}>(url)
}
...
// 批量搜索并订阅公众号
export const batchSubscribe = (companies: string[], auto_add: boolean = false) => {
  return http.post<any>('/wx/mps/batch_subscribe', { companies, auto_add })
}
